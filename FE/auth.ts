  ...
  const refreshPromise = (async () => {
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Silent refresh failed:", errorMsg)
      ...
      async authorize(credentials) {
        ...
        try {
          if (credentials.token && credentials.refreshToken) {
            // OAuth2 Login flow (JWT generated by backend)
            const tokenValue = credentials.token as string;
            const refreshTokenValue = credentials.refreshToken as string;
          ...
          return {
            ...
            provider: (data as any).provider || "LOCAL",
          }
        } catch (error) {
          throw error
        }
