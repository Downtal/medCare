...
export default function OrdersPage() {
  ...
  return (
    <>
        ...
        <CardContent className="p-6 space-y-6 bg-slate-50/30">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24">
              ...
              <p className="text-slate-500 font-bold animate-pulse">Đang tải đơn hàng...</p>
            </div>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    {order.status === 'PENDING' && order.paymentMethod === 'VNPAY' && (
                      <Button
                        onClick={async () => {
                          try {
                            const res = await fetch(`${getApiBaseUrl()}/payment-service/api/payment/create`, {
                              method: 'POST',
                              headers: {
                                'Content-Type': 'application/json',
                                'Authorization': `Bearer ${session?.user?.accessToken}`
                        ...
                        className="rounded-full bg-orange-500 hover:bg-orange-600 px-8 h-11 font-bold text-sm shadow-lg shadow-orange-500/20 transition-all"
                      >
                        Thanh toán ngay
                      </Button>
                    )}
                    {order.status === 'PENDING' && (
                      <Button
                        ...
                        variant="ghost"
                        className="rounded-full bg-red-600 hover:bg-red-700 px-8 h-11 font-bold text-sm shadow-lg shadow-red-500/20 transition-all hover:-translate-y-0.5 active:translate-y-0 text-white"
                      >
                        Hủy đơn
                      </Button>
                    )}
                    <Button
                      ...
                      className="rounded-full bg-blue-600 hover:bg-blue-700 px-8 h-11 font-bold text-sm shadow-lg shadow-blue-500/20 transition-all hover:-translate-y-0.5 active:translate-y-0"
                    >
                      Mua lại
                    </Button>
                  </div>
      ...
      <Dialog open={!!selectedOrderToCancel} onOpenChange={(open) => !open && setSelectedOrderToCancel(null)}>
        <DialogContent className="sm:max-w-[450px] rounded-[32px] p-0 overflow-hidden border-none shadow-2xl">
          <DialogHeader className="p-8 bg-red-600 text-white relative overflow-hidden text-center">
            <div className="absolute right-0 top-0 opacity-10 transform translate-x-4 -translate-y-4">
              <X className="h-32 w-32 fill-current" />
            </div>
            <DialogTitle className="text-2xl font-black">Xác nhận hủy đơn</DialogTitle>
            <p className="text-red-50/80 font-medium text-sm mt-2">Bạn có chắc chắn muốn hủy đơn hàng này không?</p>
          </DialogHeader>
          ...
          <div className="p-8 space-y-6 bg-white text-center">
            ...
            <div>
              <p className="text-slate-500 text-sm font-bold uppercase tracking-widest">Mã đơn hàng</p>
              <p className="text-xl font-black text-slate-900">#{selectedOrderToCancel?.orderCode}</p>
            </div>
            <p className="text-slate-500 text-sm italic">Hành động này không thể hoàn tác.<br />Voucher sẽ được hoàn lại.</p>
            ...
            <DialogFooter className="flex flex-col gap-3 sm:flex-col">
              <Button
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch(`${getApiBaseUrl()}/order-service/api/orders/${selectedOrderToCancel.orderCode}/cancel`, {
                      method: 'POST',
                      headers: {
                        'Authorization': `Bearer ${session?.user?.accessToken}`
                      }
                ...
                className="w-full h-14 rounded-full bg-red-600 hover:bg-red-700 text-base font-black shadow-lg shadow-red-500/20 transition-all active:scale-95"
              >
                Hủy đơn ngay
              </Button>
              <Button
