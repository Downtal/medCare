...
import { Clock, Gift, Tag, TrendingUp, Ticket, Info, Layers } from "lucide-react"
                    ...
                    <div className="bg-white rounded-[2rem] overflow-hidden shadow-lg shadow-slate-200/50 border border-slate-100 flex flex-col h-full transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                      ...
                      <div className="p-6 flex-1 flex flex-col bg-white">
                        ...
                        <div className="space-y-2 mb-6 text-xs text-slate-500 font-medium">
                          ...
                          {/* Scope Information */}
                          <p className="flex items-center gap-2 text-blue-600 font-bold mt-1">
                            <Layers className="h-3 w-3" />
                            {voucher.applicableProductId ? (
                              <span>Áp dụng cho: <span className="">{products.find((p: any) => p.id === voucher.applicableProductId)?.name || "Sản phẩm cụ thể"}</span></span>
                            ) : voucher.applicableCategoryId ? (
                              <span>Áp dụng cho: <span className="">{flatCategories.find((c: any) => c.id === voucher.applicableCategoryId)?.name || "Danh mục cụ thể"}</span></span>
                            ) : (
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-50">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              className={cn("flex-1 rounded-xl h-10 text-[10px] font-black uppercase tracking-wider shadow-md transition-all duration-300",
                                savedVouchers.some((sv: Voucher) => sv.code === voucher.code) 
