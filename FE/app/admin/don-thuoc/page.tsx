...
export default function AdminPrescriptionsPage() {
  ...
  const columns: ColumnDef<PrescriptionResponse>[] = [
    ...
    },
    {
      ...
      header: "Bệnh viện / Bác sĩ",
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-bold text-slate-800 truncate max-w-[200px]">{row.original.hospitalName || "Chưa xác định"}</span>
          <span className="text-[11px] text-slate-500">{row.original.doctorName || "Bác sĩ chưa rõ"}</span>
        </div>
      )
    },
    {
    ...
    <div className="p-10 space-y-8 max-w-[1600px] mx-auto min-h-screen">
      ...
      <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-4">
        <div className="relative">
          ...
          <Input
            placeholder="Tìm theo ID, tên bệnh viện, bác sĩ..."
                ...
                <div className="p-10 flex flex-col bg-white overflow-y-auto custom-scrollbar-slim">
                  ...
                  <div className="space-y-6 flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          ...
                          <p className="font-bold text-slate-800">{selectedPrescription.hospitalName || "---"}</p>
                       </div>
                       <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1">
                       ...
                       <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          ...
                          <p className="font-bold text-slate-800">{selectedPrescription.expiryDate || "---"}</p>
                       </div>
                       <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1">
                             <User className="w-3 h-3" /> User ID
                          </p>
                          <p className="font-bold text-slate-800">#{selectedPrescription.userId}</p>
                       </div>
                    </div>
                    ...
                    <div className="mt-8 space-y-4">
                      ...
                      {parsedExtractedData ? (
                        <div className="space-y-3">
                           {parsedExtractedData.mapped_medicines?.map((med: any, idx: number) => (
                             <div key={idx} className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:border-blue-200 transition-all group">
                                <div className="flex justify-between items-start">
                                   <div>
                                      <p className="font-black text-slate-800">{med.original_name}</p>
                                      <p className="text-xs text-slate-500 font-medium italic mt-1">{med.dosage}</p>
                                   </div>
                                   {med.matched_product ? (
                                      <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 font-bold rounded-lg text-[10px]">
                                        Khớp: {med.matched_product.name}
                                      </Badge>
                                   ) : (
                                      <Badge className="bg-amber-50 text-amber-600 border-amber-100 font-bold rounded-lg text-[10px] flex items-center gap-1">
                                        <AlertCircle className="w-3 h-3" /> Không khớp
                                      </Badge>
                                   )}
                                </div>
                             </div>
                  ...
                  <div className="mt-10 pt-10 border-t border-slate-100 flex items-center gap-4">
                     ...
                     <Button 
                        ...
                        onClick={() => updateStatusMutation.mutate({ id: selectedPrescription.id, status: 'APPROVED' })}
                        disabled={updateStatusMutation.isPending}
                     >
                        PHÊ DUYỆT ĐƠN THUỐC
                     </Button>
